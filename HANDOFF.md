...
> ⚠️ **פגם ידוע שלא תוקן: `opened_at` של 132 העסקאות המשוחזרות הוא 02/08**,
> כי `open_trades_for_expiry` קובע `datetime.now()`. ה-P&L **תקין** (מחיר
> הכניסה הגיע מהשרשרת ההיסטורית הנכונה), אבל `expiry_date − opened_at` יוצא
> שלילי עד ‎-40 ימים. כל ניתוח של תקופת החזקה על התיקים האלה שגוי.
> `SELECT count(*) FROM paper_trades WHERE opened_at::date > expiry_date` → 132.
