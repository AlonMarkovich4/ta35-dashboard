...
> 3. **הבעיה הפתוחה הגדולה** — אין מחירים סחירים בארכיון (סעיף 11).
>
...
## 11. באג הסילוק ואי-מדידות המחיר — 06/08/2026
...
### 11.2 🔴 84% מהמחירים בארכיון אינם סחירים

זו הבעיה שחוסמת הכל, וגדולה מבאג הסילוק.
...
המילוי שלנו הוא `lastrate` — ציטוט אינדיקטיבי מעופש. במקומות שבהם כן נסחרה עסקה
(n=31), הפער בין `lastrate` למחיר שנסחר בפועל הוא **43% חציוני מערך הרגל**
(ממוצע 127%, רבעון עליון 83%). ואלה דווקא הרגליים ה**נזילות**.

### 11.3 ⛔ המנוף היחיד ששרד — ולא שרד תמחור
...
ואז הוסף מחיר הכניסה:

| קנס מילוי | ROC באופק 7 ימים |
|---|---|
| 0% (רק הקצה הרע של הטווח שנסחר) | +9.2% |
| **5%** | **‎-1.0%** |
| 10% | ‎-11.1% |

**היתרון מתאפס בקנס של 5%, והקנס הנצפה הוא 43%.** אין יתרון נמדד.
...
**חסם יחיד: אין מחירי bid/ask בארכיון.** בלעדיהם כל מחקר נוסף — כולל תיק תזמון,
כולל כיול `floor`, כולל `implied_move` — ייתן מספרים משכנעים שאינם ניתנים
להבחנה מרעש. אי-הוודאות בתמחור גדולה מכל אפקט שחיפשנו.

**פתוח למשתמש / למתכנת השני:**
1. האם הפיד יכול למשוך `bid`/`ask` ולא רק `lastrate`? אם כן — זה פותח הכל.
2. אם לא — לצמצם את המערכת לסטרייקים שבהם באמת נסחרות עסקאות. זה משנה אילו
   מבנים בכלל אפשר לבנות.
3. להעביר את באג הסילוק (`_fetch_settlement_price`) — עדיין לא הועבר.
