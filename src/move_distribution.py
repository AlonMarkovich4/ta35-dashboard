...
def daily_volatility(
    ...
    **הפקיעה האחרונה**, והמדידה על 10 שנים הראתה שהאות הזה חלש (AUC 0.600)
    מול סטיית תקן יומית (0.657).

    ⛔ **אבל ה-0.657 הוא ארטיפקט של תקופה — אל תסתמכו עליו.**
    פילוח לפי משטר (אופק 5 מושבים, סף כאוס 2.5%):

        2016–2021           n=1045   AUC 0.712   (chaos 17.0%)
        2022–2026           n= 922   AUC 0.530   (chaos 32.0%)
        מ-08/2024 (חי)      n= 412   AUC 0.507   (chaos 34.5%)

    ה-0.657 על העשור נובע מהניגוד בין 2017 הרגועה (6.2% כאוס) ל-2020 (37.7%),
    ואינו משוחזר באף שנה בודדת. block bootstrap על המשטר החי: CI95
    [0.430, 0.570], P(AUC>0.60)=0.005.

    **במשטר הנוכחי לאות אין כושר הפרדה.** גם EWMA(λ=0.94) — השדרוג הסטנדרטי —
    יוצא 0.5114, ופערי פתיחה 0.4467. הקו סגור: לא מחוסר בדיקה אלא כי אין מה
    למצוא. **המנוף אינו ניבוי משטר אלא גודל פוזיציה ובחירת מבנה** (ראה
    margin_selector.required_hold).

    השדה נשמר לתיעוד בלבד ואינו משפיע על שום בחירה.
