...
- **`is_chain_fresh` — ברירת מחדל 0 = "נמשכה היום".** זו הרמה לכל מי שכותב ל-DB.
  ✅ **נוסף ל-`vwap_trader` ב-01/09/2026** (תיקים 9,10) — הוא נכתב אחרי תשעה באב
  ולא קיבל את השער, והיה היחיד שיכול לפתוח על שרשרת בת ימים. יחד איתו נוסף גבול
  `recommended_at::date = today` בבורר וב-`_latest_recommendation`. שלושת השערים
  חופפים בכוונה — אל תסיר אחד כי "השני תופס". פירוט: HANDOFF 6ד.
